...
% Transformation from Inertial to k
T_Ik = kin.T_Ik;
% Rotation from Inertial to k
R_Ik = kin.R_Ik;
% rotation axis in respective k frame
k_n_k = kin.k_n_k;
% Translation vector from inertial frame to end effector
I_r_Ie = kin.I_r_Ie;

% Location of CoM of each link in the respective link frame
k_r_ks = params.k_r_ks;

% All the position jacobians of all the links (w.r.t. CoM)
jac.I_Jp_s = cell(6,1);
% Analogous Rotation Jacobians
jac.I_Jr = cell(6,1);
% End effector jacobians both position and rotation
...
for k=1:6
    ...
    % translational jacobian at the center of gravity s in frame I – first
    % build joint distance of CoM k and then build transl. jacobian
    ...
    if k == 1
        ...
        %further jacobians depend on previous gen coord.
